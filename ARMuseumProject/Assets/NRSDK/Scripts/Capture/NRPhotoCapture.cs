...
namespace NRKernal.Record
{
    ...
    public class NRPhotoCapture : IDisposable
    {
        ...
        public void TakePhotoAsync(OnCapturedProcessCallback onCapturedProcessCallback)
        {
            try
            {
                var capture = m_CaptureContext.GetBehaviour();
                ((NRCaptureBehaviour)capture).DoAsyn(onCapturedProcessCallback);
            }
            catch (Exception)
            {
                throw;
        ...
        // User add
        public delegate void OnCapturedProcessCallback(byte[] bytes);
    }
}
